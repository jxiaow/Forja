...
5. `run` 是长生命周期命令，必须后台启动；确认启动或快速失败后立刻反馈，不等待客户端退出
...
- **run 用后台进程**：`qt-pilot run --execute` 会先编译再启动程序，程序不会自动退出；必须后台启动，短暂等待确认进程进入构建/启动阶段或快速失败，然后立刻向用户反馈
- **run 禁止阻塞等待**：不要用阻塞式 shell 调用等待 `qt-pilot run --execute` 完整结束；需要持续观察日志时，先把后台进程 PID 和日志位置反馈给用户，再等用户明确要求继续看
...
在 Codex/agent shell 工具中运行客户端时，优先用后台模板，避免卡住当前回合：

```powershell
$log = Join-Path (Get-Location) ".qtpilot\run.log"
$proc = Start-Process -FilePath "powershell" -ArgumentList @("-NoProfile", "-Command", "qt-pilot run --execute *> `"$log`"") -WorkingDirectory (Get-Location) -WindowStyle Hidden -PassThru
Start-Sleep -Seconds 3
if ($proc.HasExited) { Get-Content -LiteralPath $log -Tail 80; exit $proc.ExitCode }
"qt-pilot run started: pid=$($proc.Id), log=$log"
```

如果用户明确要求在可见终端中观察实时输出，再按用户指定的终端方式执行；否则不要阻塞等待客户端退出。
