...
5. 写入或更新 `~/.local/bin/forja`
6. 从非安装目录执行 `~/.local/bin/forja --version` 验证
...
bootstrap 使用 `~/.local` npm 用户级 prefix；新入口验证成功后删除旧的 `~/.forja/bin/forja`。若 `~/.local/bin` 尚未进入 PATH，返回 warning 和配置提示。
