...
| 分类 | 含义 | 默认处理 |
| --- | --- | --- |
| `preservedTracked` | 远端本来就有的 tracked dirty，且不属于 Compilot sync overlay | branchSync 时 stash/pop 保留 |
| `overlayTracked` | 上一次 Compilot sync 上传后造成的 tracked dirty | branchSync 前恢复到 git baseline，再按本次 sync 重放 |
| `overlayUntracked` | 上一次 Compilot sync 上传的 untracked 文件 | branchSync 前删除该 overlay 文件，再按本次 sync 重放 |
...
| `unknownUntracked` | 远端已有但不由 Compilot 记录的 untracked 文件 | 默认保留；阻塞 git 操作时失败 |
...
```text
~/.compilot/remote-state/<targetId>/overlay.json
~/.compilot/remote-state/<targetId>/underlay/
```

manifest 是“Compilot 在该远端工作区留下过哪些 overlay 文件”的记录，不是 run-state，也不替代现有 sync mtime state。

每次 remote sync 成功后：

1. 以本次上传结果更新 manifest
2. 按 repo 区分本地 tracked dirty、untracked 上传文件和 tracked deletion
3. 不把普通 `compilot qt sync` 的历史文件自动视为 remote overlay，除非 remote sync 明确接管该写入路径
...
```bash
git stash push -m "compilot-remote-preserve" -- <tracked paths>
```
...
| 状态 | 真相来源 | 用途 |
| --- | --- | --- |
| sync mtime state | 本地 `~/.compilot/sync/` | 普通 sync 的上传优化；remote git repo 不以它决定 overlay 是否重放 |
| remote overlay manifest | 远端 `~/.compilot/remote-state/<targetId>/` | 下次 branchSync 前清理上次 remote sync overlay |
| remote underlay backup | 远端 `~/.compilot/remote-state/<targetId>/underlay/` | 恢复被 overlay 覆盖的远端 preserved dirty |
| remote pipeline state | 本地 `~/.compilot/remote-state/` | 最近阶段、失败点、server/remotePath 摘要 |
| remote run-state | 远端 compilot workspace state | `remote qt ps/stop` 的 PID/logFile 真相 |
| remote target lock | 远端用户目录 | 阻止多个本地客户端并发修改同一远端 checkout |

本地 remote pipeline state 不缓存远端进程存活结论。`remote qt ps` 必须向远端 compilot 查询。
