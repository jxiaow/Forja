...
本文补充 `docs/remote-deploy-v3.md` 中 `compilot remote bootstrap` 的 artifact、版本兼容、安装和清理策略。
...
```text
dist/compilot-<version>/cli/compilot-cli-<version>.tgz
```
...
第一版要求本地 launcher 和远端 compilot exact version 一致。

- `remote test` 检测远端 version 不一致时失败
- `remote test --bootstrap` 可以安装 exact version 后复测
- `remote bootstrap` 安装成功后必须执行 `~/.compilot/bin/compilot --version`
...
1. 上传 tgz 到 `~/.compilot/bootstrap/compilot-cli-<version>.tgz.tmp`
2. 校验 sha256
3. rename 为 `compilot-cli-<version>.tgz`
4. `npm install -g --prefix ~/.compilot/runtime/<version> <tgz>`
5. 验证 `~/.compilot/runtime/<version>/bin/compilot --version`
6. 原子切换 `~/.compilot/bin/compilot` symlink 或 shim
7. 验证 `~/.compilot/bin/compilot --version`

任何一步失败都保留旧 `~/.compilot/bin/compilot`。
...
```json
{
  ...
  "mode": "remote",
  "version": "0.7.42",
  "artifact": "dist/compilot-0.7.42/cli/compilot-cli-0.7.42.tgz",
  "sha256": "<sha256>",
  "remoteBin": "~/.compilot/bin/compilot",
  "stages": [
