...
| 命令 | 目的 | 默认退出语义 |
| --- | --- | --- |
| `forja remote test` | 判断远程通道是否能用于后续动作 | 任一必需层失败则 `ok=false` |
| `forja remote test --bootstrap` | 在 forja 缺失/不兼容时允许执行 bootstrap 后再测试 | bootstrap 或复测失败则 `ok=false` |
| `forja remote status` | 盘点当前远程配置、目标状态和下一步建议 | 查询本身成功则 `ok=true`，具体能力看 `overall` |
| `forja remote unlock --lock-id <id> --force` | 删除匹配 lock-id 的远端 stale lock | 删除成功才 `ok=true` |
| `forja remote qt/sdk status` | 在远端 workspace 下运行远端 Qt/SDK status | 返回 bridge 结果 |
| `forja remote qt/sdk init/use` | 在远端初始化或选择 Qt/SDK 用户配置 | 返回 bridge 结果 |
| `forja remote qt build/clean/qmake/run` | prepare 远端 workspace 后执行远端 Qt 动作 | prepare 和远端 action 都成功才 `ok=true` |
| `forja remote qt stop/ps` | 直接桥接远端 Qt 后台运行状态管理，不做 branchSync/sync | bridge 成功才 `ok=true` |
| `forja remote sdk build/rebuild/clean` | prepare 远端 workspace 后执行远端 SDK 构建类动作 | prepare 和远端 action 都成功才 `ok=true` |
| `forja remote qt/sdk restore --repo <repo> -- <paths...>` | 路径级恢复远端 tracked 文件到远端当前 git HEAD，并同步清理 overlay/underlay state | 成功恢复并清理状态才 `ok=true` |
...
| `remoteForja` | 远端 forja 存在且版本兼容 | 是 |
| `repoDiscovery` | 本地和远端 repo 模式可判定 | 是 |
| `baselinePrecheck` | 本地分支、upstream、可拉取 commit 满足 branchSync 前提 | `git` repo 必需 |
| `targetQt` | 远端 `forja qt status --json` ready | Qt build/run/qmake/clean 必需 |
| `targetSdk` | 远端 `forja sdk status --json` ready | SDK build/rebuild/clean 必需 |
| `targetLock` | 远端 checkout 当前没有被其他 pipeline 持有 | 会修改 checkout 的动作必需 |

`remote test` 第一版只要求 `syncConfig`、`ssh`、`remotePlatform`、`remotePath`、`remoteForja`。它不跑 branchSync、不跑 sync、不要求 Qt/SDK ready。
...
- `forja remote build-order status --json`
- `forja remote transfer status --json`
- 可选 `forja remote test --bootstrap --json`
- `forja remote qt|sdk status --json`
- 可选 `forja remote qt|sdk build --json`
- 可选 `forja remote qt run --detach --json`
- 可选 `forja remote qt ps --json`
- 可选 `forja remote qt stop --json`
- 最后再次 `forja remote doctor --json`，确认没有 stale lock 或 readiness 退化
...
- 远端已有兼容 forja，或传入 `--bootstrap --yes` 并且本地已有 bootstrap artifact
- Qt/SDK 远端配置已可由 `forja remote qt|sdk status --json` 验证
...
```bash
npm run remote:smoke -- --target both --build --run-detach --stop --execute --yes --json-dir /tmp/forja-remote-smoke
```

`--json-dir` 只保存每个 step 的 stdout/stderr 和退出码，便于对照 pipeline JSON；它不是远端状态源。

手工排查时，`forja remote status` 的普通文本输出会给出 server、remotePath、remoteForjaBin、buildOrder、transfer 和 next action 摘要。JSON 模式仍是 AI/脚本集成的稳定入口。

## Status JSON

`forja remote status --json` 返回快照结构：
...
```json
{
  ...
  "layers": [
    ...
    },
    {
      ...
      "nextActions": ["forja remote qt init"]
    }
  ],
  "targets": {
    "qt": {
...
| value | 含义 |
| --- | --- |
| `ready` | transport、remote forja、repo precheck 和已知 target 都可用 |
...
`forja remote test --json` 是更窄的布尔结果：
...
```json
{
  ...
  "action": "test",
  "mode": "remote",
  "failedLayer": "remoteForja",
  "diagnostics": [
    {
      "level": "error",
      "message": "remote forja is not installed"
    }
  ],
  "nextActions": ["forja remote bootstrap"]
}
```
...
`remote qt/sdk status/init/use` 是 bridge 命令。它们解析本地 sync target，通过 SSH 在远端 workspace 下执行对应的远端 forja 命令。

参数规则：

- `forja remote qt use --mode release --arch x64`
- `forja remote qt use --project app.pro`
- `forja remote sdk use -- <sdk-specific args>`

remote CLI 只消费 `remote`、`qt|sdk`、`status|init|use` 这三段和 remote 自己的通用参数；其余参数原样传给远端 `forja qt|sdk`。
...
```json
{
  ...
  "target": "qt",
  "mode": "remote",
  "remoteCommand": "forja qt use --mode release --json",
  "result": {
    "ok": true,
    "action": "use"
  },
  "diagnostics": [],
  "nextActions": ["forja remote qt status"]
}
```
