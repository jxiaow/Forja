...
**职责**：轻量只读状态查看。不做主�?SSH 探测、不做完整工具链探测、不修改配置�?
**语法**�?```
forja status [--workspace <path>] [--json] [--lang <locale>]
```

**行为**�?1. 解析 workspace�?2. 读取 active target 元数据�?3. 读取 readiness 判断所需的最小设置，不返回完整配置摘要�?4. �?active target 时：扫描候选数量，不猜测，返回 `forja list` + `forja use target --project <path>`�?5. �?active target 时：验证项目存在、mode/arch 有效、runAt 配置完整�?6. `runtime` 字段始终返回，读取本�?runState 文件（轻量操作）�?7. `nextAction` 可提�?`forja use target --project <path>` 切换目标、`forja use qt|sdk ...` 修改配置�?8. status 只使用本地可读配置、缓存的 remote metadata、已�?lock/runState 摘要；不会为了判断状态主动发�?SSH、上传、bootstrap 或远程命令。需要实�?SSH/版本/路径可达性验证时指向 `forja doctor --remote`�?
**吸收的旧命令**�?`forja qt status`、`forja sdk status`、`forja remote status`、`forja sync status`、`forja qt ps`、`forja remote qt ps`

**注意**：以下旧命令已归�?`forja list remote`（纯配置列举）：
`forja remote workspace status`、`forja remote forja-bin status`、`forja remote build-order status`、`forja remote transfer status`

**�?`list` 的边�?*�?- `status` 回答"能不能用"，只输出 readiness 判断所需的最小摘要（server/remotePath/workspaceMode）�?- `status` 可以�?`nextAction` 中提示查看或切换配置的命令，但不展开配置内容�?- `list config` 回答"当前保存了哪些配置摘�?，输�?Qt/SDK/Sync/Remote 配置摘要�?- `list remote` 回答"配了什�?，输出全部远程配置细节（workspace/bin/build-order/transfer/repos）�?- `list targets/env/servers/remote` 回答可选项和专项明细�?
**VSCode 映射**�?
...
| `forja.remote.transfer.status` | `forja.list` / `forja.status` | 纯配置详情归 list，readiness 摘要�?status |
| `forja.remote.workbench` | `forja.status` / QuickPick | 工作台入口收敛为状�?+ 操作建议 |
...
|------|-------|------------|---------|---------|---------|--------------|
| target | 项目存在、配置完�?| 有配置但未深度验�?| 配置有但有问题（�?Makefile 不匹配） | 项目文件不存�?| 无法判断（配置损坏） | 未选择 |
| toolchain | Qt/VS/jom/make 就绪 | 有路径但未验证可用�?| 路径无效或版本不�?| 缺少工具�?| 无法判断 | �?|
| sync | server + remotePath 完整 | 有配置但未验�?SSH | 配置有阻塞（如服务器被删�?| 缺少 server/path | 无法判断 | 未配�?|
| remote | SSH + forja + path 就绪 | 有配置但未连�?| SSH/forja 失败或被�?| 缺少 server | 无法判断 | runAt=local |
| runtime | 进程运行�?| �?| �?| �?| 无法查询 | 未运行（无进程时�?|

**Status 诊断�?*�?
| key | level | EN message | ZH message |
|-----|-------|-----------|-----------|
| `sts.workspaceNotFound` | error | Workspace does not exist: {path} | Workspace 不存�? {path} |
...
| `sts.targetsFound` | info | Found {qtCount} Qt and {sdkCount} SDK targets, none selected | 发现 {qtCount} �?Qt 目标�?{sdkCount} �?SDK 目标，未选择 |
| `sts.projectFileMissing` | error | Project file does not exist: {project} | 项目文件不存�? {project} |
...
| `vsNotFoundDetail` | error | VS dev environment not found (vsDevShell) | 未找�?VS 开发环�?(vsDevShell) |
| `jomNotFound` | warning | jom not found (optional, recommended for faster builds) | 未找�?jom（可选，推荐安装以加速构建） |
| `makeNotFound` | error | make not found | 未找�?make 工具 |
| `remoteNoServer` | error | runAt=remote but no server configured | runAt=remote 但未配置服务器和远程路径 |
| `remoteForjaBinDefault` | info | Remote Forja bin not configured, will use default | 远程 Forja 二进制未配置，将使用默认�?|
| `remotePathNotConfigured` | error | Remote path not configured: {path} | 未配置远程路�? {path} |
| `sts.syncServerNotFound` | error | Sync server "{server}" does not exist | 同步服务�?"{server}" 不存�?|
...
_�?workspace_�?```json
{
    ...
    "diagnostics": [
        { "level": "warning", "message": "No active target selected", "fix": "forja use target" }
    ],
...
_从未初始化_�?```json
{
    ...
    "diagnostics": [
        { "level": "warning", "message": "Not initialized, no config found", "fix": "forja use target" }
    ],
    "nextAction": "forja use target"
}
```
...
_配置文件损坏_�?```json
{
    ...
    "diagnostics": [
        { "level": "error", "message": "Config file parse failed: ~/.forja/projects/<hash(workspace:qt)>.json �?Unexpected token } in JSON", "hint": "File may be corrupted, delete and re-run forja use target", "params": { "file": "~/.forja/projects/<hash(workspace:qt)>.json", "detail": "Unexpected token } in JSON" } }
    ],
    "nextAction": "forja use target"
}
```

_Makefile 与当前配置不匹配_�?```json
{
    "ok": false,
    "action": "status",
    "workspace": "/path/to/workspace",
...
Error: Config file parse failed: ~/.forja/projects/<hash(workspace:qt)>.json �?Unexpected token } in JSON
  hint: File may be corrupted, delete and re-run forja use target
Next:
  forja use target
```
...
**`ok` 判定规则**�?- target/toolchain �?`blocked`、`missing`、`unknown` �?`ok: false`
- target �?`not-selected` �?`ok: false`
- remote �?`blocked`、`missing` �?`ok: false`
- `sync`、`runtime` 不影�?`ok` 值（sync 问题不阻塞本地构建）

**验证�?*�?
- `forja status --json` 在无 active target 时不猜测目标�?- `forja status --json` 始终返回 runtime 字段，且 runtime 不影�?`ok`�?- `forja status --json --lang zh` 只本地化 message/hint，不改变 readiness 枚举值和 fix 命令�?- runAt=remote �?server/path 时返�?`remote=missing` 并指�?`forja server` + `forja use remote --server`�?- `forja status --json` 不主�?SSH；远程深度检查（SSH 连通性、版本兼容、锁状态）�?`forja doctor --remote` 负责�?- 纯远程配置详情不�?status 展开，指�?`forja list remote`�?
