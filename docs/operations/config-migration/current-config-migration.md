...
> **状态：superseded。** 本文记录的 `~/.forja/projects/` 迁移方案不再执行。新版本采用全新的 workspace store，旧配置不读取、不迁移、不兼容。
...
- Current assessment: 该方案已被 `workroot-redesign` 取代，不作为当前实现依据。
- Current biggest issue: 防止旧文档再次被当作当前配置契约使用

## 2. Stage-Level Todo

### Stage 1: settingsIO 层改造（历史记录）
