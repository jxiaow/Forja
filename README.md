...
VSCode 扩展，为 Qt/C++ 项目提供构建、运行、调试支持，适用于 Windows（MSVC）和 Linux（GCC/Clang）。
...
- 配置面板：可视化管理所有构建参数
- `.pri`/`.pro` 文件监听：新建/删除 `.cpp`/`.h`/`.ui` 文件时自动提示同步
...
通过命令面板（`Ctrl+Shift+P`）搜索 `XY Qt:` 可访问所有命令：

| 命令 | 说明 |
|------|------|
| `XY Qt: 选择项目` | 手动选择 `.pro` 文件 |
| `XY Qt: 显示操作菜单` | 打开状态栏操作菜单 |
| `XY Qt: QMake` | 运行 qmake 生成 Makefile |
| `XY Qt: Build` | 编译项目 |
| `XY Qt: Clean` | 清理构建产物 |
| `XY Qt: Run` | 构建并运行 |
| `XY Qt: 停止` | 终止正在运行的程序 |
...
| 配置项 | 类型 | 说明 |
|--------|------|------|
| `xyQt.qtPath` | string | Qt 路径（留空自动检测） |
| `xyQt.vsDevShellPath` | string | `Launch-VsDevShell.ps1` 路径（留空自动检测） |
| `xyQt.selectedProject` | string | 当前选中的 `.pro` 文件（相对路径） |
| `xyQt.qmakeTarget` | string | QMake TARGET 覆盖（留空使用默认） |
| `xyQt.arch` | `x86`/`x64` | 目标架构，默认 `x86` |
| `xyQt.cStandard` | string | IntelliSense C 标准，默认 `c11` |
| `xyQt.cppStandard` | string | IntelliSense C++ 标准，默认 `c++11` |
| `xyQt.scanExcludeDirs` | string[] | 生成 IntelliSense 时额外排除的目录 |
