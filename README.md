...
Qt Pilot 在 `codex-qt-pilot-cli-design` 分支中提供了一个面向脚本和 AI 工具的 CLI 入口。它默认以 dry-run 方式返回命令计划，只有显式传入 `--execute` 才会真正执行。
...
node out/cli/index.js --json
node out/cli/index.js status --json
node out/cli/index.js build --json
node out/cli/index.js build --execute --json
node out/cli/index.js run --execute --json
```
...
- `--arch x86|x64`
- `--qt-path <qt-dir>`
- `--vs-dev-shell <Launch-VsDevShell.ps1>`
- `--target <qmake-target>`
- `--execute`
- `--save-local`
- `--json`
...
- 不写 action 时，默认执行 `status`。
- 加 `--json` 时会返回适合 AI/脚本处理的结构化结果。
- 结果中的 `candidates`、`nextActions`、`resolved` 字段可以帮助调用方在缺少项目信息时继续决策。
- `init --execute` 会在项目下写入 `.work/qt-pilot/` 本地状态。
