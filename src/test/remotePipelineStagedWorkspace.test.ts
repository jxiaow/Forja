...
test('staged prepared action runs readiness and remote action in the primary staged repo after prepare', async () => {
    ...
    const runner: RemoteRunner = {
        async run(command: string) {
            commands.push(command);
            if (command.includes('$HOME/.local/bin/forja')) {
                if (!prepared) {
    ...
    try {
        ...
        assert.equal(commands.some(command => command.includes("cd '/home/xw/workspace/dev'") && command.includes('$HOME/.local/bin/forja')), false);
    } finally {
        fs.rmSync(fixture.root, { recursive: true, force: true });
        fs.rmSync(configDir, { recursive: true, force: true });
        if (oldConfigDir === undefined) { delete process.env.FORJA_CONFIG_DIR; }
...
test('staged prepared action uses mapped remote repo name for primary action path', async () => {
    ...
    try {
        ...
        assert.equal(commands.some(command => command.includes(`cd '${stagedWorkspace}'`) && command.includes('$HOME/.local/bin/forja')), false);
...
test('staged prepared action preserves remote forja build failures instead of shell fallback', async () => {
    ...
    const runner: RemoteRunner = {
        async run(command: string) {
            commands.push(command);
            if (command.includes('$HOME/.local/bin/forja') && command.includes("'status'")) {
                return { exitCode: 0, stdout: '{"ok":true,"diagnostics":[],"nextActions":[]}', stderr: '' };
            }
            if (command.includes('$HOME/.local/bin/forja') && command.includes("'build'")) {
                return {
                    exitCode: 1,
