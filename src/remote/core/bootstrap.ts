...
export async function executeRemoteBootstrap(options: ExecuteRemoteBootstrapOptions): Promise<ExecuteRemoteBootstrapResult> {
    ...
    const remoteBin = '$HOME/.compilot/bin/compilot';
    ...
    const remoteArtifact = `.compilot/bootstrap/${artifactName}`;
    const remoteArtifactForShell = homePath('.compilot', 'bootstrap', artifactName);
    const runtimePrefix = homePath('.compilot', 'runtime', version);
    const runtimeBin = homePath('.compilot', 'runtime', version, 'bin', 'compilot');
    const publicBin = homePath('.compilot', 'bin', 'compilot');

    const prepare = await options.runner.run(`mkdir -p ${homePath('.compilot', 'bootstrap')} ${homePath('.compilot', 'runtime', version)} ${homePath('.compilot', 'bin')}`, 10000);
    ...
    if (runtimeVersion.exitCode !== 0 || runtimeVersionText !== version) {
        diagnostics.push({ level: 'error', message: `远端 compilot runtime 版本不匹配: ${runtimeVersionText || trim(runtimeVersion.stderr)}` });
        return failure(artifact, remoteBin, stages, diagnostics);
    }
    ...
    if (link.exitCode !== 0) {
        diagnostics.push({ level: 'error', message: trim(link.stderr) || '远端 compilot 链接创建失败' });
        return failure(artifact, remoteBin, stages, diagnostics);
    }
    ...
    if (publicVersion.exitCode !== 0 || publicVersionText !== version) {
        diagnostics.push({ level: 'error', message: `远端 compilot 入口版本不匹配: ${publicVersionText || trim(publicVersion.stderr)}` });
        return failure(artifact, remoteBin, stages, diagnostics);
    }
