...
export async function buildRemoteDoctor(options: BuildRemoteDoctorOptions): Promise<RemoteDoctorResult> {
    ...
    const remoteCompilot = status.layers.find(layer => layer.name === 'remoteCompilot');
    const canBootstrap = remoteCompilot?.ok === false;
    const autoFixes: RemoteDoctorAutoFix[] = [{
        name: 'bootstrap',
        available: canBootstrap,
        command: 'compilot remote test --bootstrap',
        reason: canBootstrap ? undefined : 'remoteCompilot 检查未失败或尚未到达该检查'
    }];
    ...
    } else if (canBootstrap && !nextActions.includes('compilot remote test --bootstrap')) {
        nextActions.push('compilot remote test --bootstrap');
    }
