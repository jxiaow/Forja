...
export async function runRemoteCli(argv: string[]): Promise<void> {
    ...
    try {
        ...
        if (options.action === 'bootstrap') {
            ...
            if (!artifact.ok) {
                process.exitCode = 1;
                writeOutput({ action: 'bootstrap', mode: 'remote', remoteBin: '$(npm prefix -g)/bin/forja', ...artifact }, options.json);
                return;
            }
...
function formatRemoteStatus(out: {
    ...
    if (out.remoteSettings) {
        lines.push('remoteForjaBin: ' + (out.remoteSettings.remoteForjaBin || '$(npm prefix -g)/bin/forja'));
