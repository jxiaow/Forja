...
function buildSdkEnvSection(data: TemplateData): string {
    ...
    h += '<div class="env-card"><div class="ech">';
    h += `<span class="sd ${data.sdkVsInstall ? 'dok' : 'dwn'}"></span>`;
    h += '<span class="ect">VsDevCmd</span>';
    h += `<span class="ecb ${data.sdkVsInstall ? 'bok' : 'bwn'}">${data.sdkVsInstall ? '已配置' : '未配置'}</span>`;
    h += '</div>';
    h += `<div class="ecp">${esc(data.sdkVsInstall || '留空使用自动检测')}</div>`;
    h += '<div class="eca"><button class="btn btn-sm env-toggle-btn"';
